function params = PET_AI_Step5(params)
    ...
    %% FORWARD DEFORMATION
    images = {params.MNI_aligned.PET, params.flipped.PET};
    ...
    [~, n, e] = fileparts(params.MNI_aligned.PET);
    images{1} = strrep(params.MNI_aligned.PET, [n e], ['w' n e]);
    [~, n, e] = fileparts(params.flipped.PET);
    images{2} = strrep(params.flipped.PET, [n e], ['w' n e]);
    ...
    [~, nnPET, ~] = fileparts(params.MNI_aligned.PET);
    [~, nnflipPET, ~] = fileparts(params.flipped.PET);
...
function generate_report_template(report, params)
    ...
    if isempty(report) || isempty(fieldnames(report))
        ...
        fprintf(fid, '<p style="font-family: Aptos; font-size: 12pt">No clusters found.</p>');
...
function perform_burnin(params)
    ...
    t1_mni_hdr = spm_vol(params.MNI_aligned.T1);
    t1_mni_vol = spm_read_vols(t1_mni_hdr);

    % reslice Z_data (currently on "MNI" voxel grid) back into the original T1 voxel grid    
    ...
    for t_idx = 1:length(thresholds)
        ...
        switch t_idx % - (length(thresholds) - 3)
            ...
            case 2
                burnin_vol(mask) = nanmin(t1_mni_vol(:)); % black burn-in
            case 3
                ...
                gm_int = nansum(t1_mni_vol(gm_vol > 0) .* gm_vol(gm_vol > 0)) / nansum(gm_vol(gm_vol > 0));
                wm_img = params.c2T1;
                wm_vol = spm_read_vols(spm_vol(wm_img));
                wm_int = nansum(t1_mni_vol(wm_vol > 0) .* wm_vol(wm_vol > 0)) / nansum(wm_vol(wm_vol > 0));
